    ...
    <header className="sticky top-0 z-40 bg-[#FAF9F5] border-b-4 border-[#0F172A]">
      <div className="max-w-7xl mx-auto px-4 py-2.5 flex items-center justify-between gap-3">
        ...
        <Link to={navItems[0]?.path || '/'} className="flex items-center gap-2 shrink-0">
          <span className="bg-[#15803D] text-white p-1.5 border-2 border-[#0F172A] shadow-[2px_2px_0px_#0F172A] text-base">🌾</span>
          <div className="hidden sm:flex flex-col leading-none">
            <span className="text-sm font-black uppercase">CROP RESIDUE <span className="bg-[#EAB308] px-1 border border-[#0F172A]">PORTAL</span></span>
            <span className="text-[9px] font-extrabold uppercase tracking-widest text-[#15803D]">{role.toUpperCase()} PORTAL</span>
          </div>
        </Link>
        ...
        <div className="flex items-center gap-2 shrink-0">
                    ...
                    notifications.map((n) => (
                      <div
                        ...
                        className={`p-3 border-2 border-black cursor-pointer transition-colors ${
                          n.is_read ? 'bg-gray-100 opacity-75' : 'bg-yellow-50 font-bold border-l-4 border-l-green-600'
                        }`}
                      >
          ...
          <div className="relative" ref={profileRef}>
            <button
              ...
              title="My Profile"
            >
              <span className={`w-7 h-7 rounded-sm flex items-center justify-center text-xs font-black border-2 border-[#0F172A] ${roleColors[role] || 'bg-gray-200'}`}>
                {initials}
              </span>
              <span className="hidden sm:block text-xs font-black uppercase max-w-[80px] truncate">{currentUser?.name}</span>
            ...
            {showProfileMenu && (
              <div className="absolute right-0 mt-2 w-52 neo-box-static bg-white z-50 shadow-[6px_6px_0px_#000] overflow-hidden">
                <div className="p-3 border-b-2 border-black bg-gray-50">
                  ...
                  <span className={`neo-badge text-[10px] mt-1 ${roleColors[role] || 'bg-gray-200'}`}>{role.toUpperCase()}</span>
                </div>

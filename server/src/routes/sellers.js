...
router.get('/profile', async (req, res) => {
  try {
    ...
    let seller = await get('SELECT * FROM sellers WHERE user_id = ?', [userId]);
    if (!seller) {
...
router.post('/kyc', async (req, res) => {
  try {
    ...
    let seller = await get('SELECT * FROM sellers WHERE user_id = ?', [user_id]);

    if (seller) {
      await run(
        `UPDATE sellers SET seller_type = ?, business_name = ?, phone = ?, address = ?, kyc_docs_url = ?, kyc_status = 'pending' WHERE id = ?`,
        [seller_type, business_name, phone, address, kyc_docs_url, seller.id]
      );
    } else {
      const sellerId = 'seller_' + Date.now();
      await run(
        `INSERT INTO sellers (id, user_id, seller_type, business_name, phone, address, kyc_status, kyc_docs_url) VALUES (?, ?, ?, ?, ?, ?, 'pending', ?)`,
        [sellerId, user_id, seller_type, business_name, phone, address, kyc_docs_url]
      );
    }

    // Also update user kyc_status
    await run('UPDATE users SET kyc_status = "pending" WHERE id = ?', [user_id]);

    const updatedSeller = await get('SELECT * FROM sellers WHERE user_id = ?', [user_id]);
...
router.get('/my-listings', async (req, res) => {
  try {
    ...
    const machines = await query('SELECT * FROM machines WHERE seller_id = ? ORDER BY created_at DESC', [sellerId]);
    const buyerListings = await query('SELECT * FROM buyer_listings WHERE seller_id = ? ORDER BY created_at DESC', [sellerId]);
    ...
    const bookings = await query(
      `SELECT b.*, f.name as farm_name, u.name as farmer_name, u.phone as farmer_phone, m.name as machine_name
       ...
       WHERE m.seller_id = ? ORDER BY b.created_at DESC`,
      [sellerId]
    );
    ...
    const connectionRequests = await query(
      `SELECT cr.*, f.name as farm_name, u.name as farmer_name, u.phone as farmer_phone, bl.buying_purpose
       ...
       WHERE bl.seller_id = ? ORDER BY cr.created_at DESC`,
      [sellerId]
    );
...
router.post('/machines', async (req, res) => {
  try {
    ...
    const machine = await get('SELECT * FROM machines WHERE id = ?', [machId]);
...
router.post('/buyer-listings', async (req, res) => {
  try {
    ...
    const listing = await get('SELECT * FROM buyer_listings WHERE id = ?', [listingId]);
